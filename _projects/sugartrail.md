---
layout: project
title: "Sugartrail"
description: "Visualising networks of companies, officers and addresses connected within Companies House"
year: 2022
tags: [OSINT]
cover_image: '/images/projects/sugartrail/candy.png'
carousel_image: '/images/carousel/sugartrail.jpg'
link: 'https://github.com/bellingcat/sugartrail'
hero_image: '/images/projects/sugartrail/candy.png'
---

![Report](/images/projects/sugartrail/regent.png)
<!-- ![Report](/images/projects/sugartrail/network.png){: .shadow } -->


[Sugartrail](https://github.com/bellingcat/sugartrail) is an open-source network analysis tool to empower OSINT researchers to navigate corporate ownership networks using data from the UK Companies House register. The tool was developed within a self-directed 2-month tech fellowship awarded by the investigative journalism group [Bellingcat](https://www.bellingcat.com/){:target="_blank" rel="noopener noreferrer"} and funded by the Alfred Landecker Foundation.

{% include youtube.html id="evPXGTj33LQ" autoplay="true" %}

**Featured in**: [Bellingcat](https://www.bellingcat.com/resources/2022/09/19/identifying-suspicious-businesses-reddit-analysis-and-tracking-russian-propaganda-here-are-the-results-of-bellingcats-first-ever-hackathon/){:target="_blank" rel="noopener noreferrer"}, [Global Investigative Journalism Network](https://gijn.org/stories/new-reporting-tools-to-archive-videos-find-qanon-networks-and-track-targets-via-online-reviews/){:target="_blank" rel="noopener noreferrer"} & [Journal of Economic Criminology](https://www.sciencedirect.com/science/article/pii/S2949791424000071){:target="_blank" rel="noopener noreferrer"}